...
from tkinter import messagebox # Added for practice tab
import ipaddress
import math # Added for practice tab

# Placeholder functions for the existing code, assuming they are defined earlier in the full document
def calculate_main():
    pass
...
# Assuming these are defined earlier in the full document
detailed_text = None # Will be assigned later
...
# ============================================================================
# TAB 3: Práctica Manual Guiada
# ============================================================================
tab3 = ttk.Frame(notebook)
notebook.add(tab3, text="Práctica Manual Guiada")

# Global variables for practice
practice_state = {
    ...
    'cidr': None,
    'network': None,
    'mode': None  # 'basic' or 'subnets'
}

def generate_practice_exercise():
    """Generate a random IP and CIDR for practice"""
    import random
...
def start_basic_practice():
    """Start basic network calculation practice"""
    practice_state['mode'] = 'basic'
    practice_state['current_step'] = 1
    
    ip, cidr = generate_practice_exercise()
    ...
    output = "╔════════════════════════════════════════════════════════════════════════════╗\n"
    output += "║                     EJERCICIO DE PRÁCTICA BÁSICA                          ║\n"
    output += "╚════════════════════════════════════════════════════════════════════════════╝\n\n"
    
    output += f"📝 EJERCICIO:\n"
    output += f"   Dada la IP: {ip}/{cidr}\n\n"
    
    output += "🎯 TU TAREA:\n"
    output += "   Calcula manualmente los siguientes valores y completa los campos:\n\n"
    
    output += "   1️⃣  MÁSCARA DE RED (formato: 255.255.255.0)\n"
    output += "   2️⃣  DIRECCIÓN DE RED (formato: 192.168.1.0)\n"
    output += "   3️⃣  DIRECCIÓN DE BROADCAST (formato: 192.168.1.255)\n"
    output += "   4️⃣  SALTO DE BLOQUE (solo el número, ej: 256)\n"
    output += "   5️⃣  PRIMER HOST UTILIZABLE\n"
    output += "   6️⃣  ÚLTIMO HOST UTILIZABLE\n\n"
    
    output += "💡 AYUDA:\n"
    output += f"   • El prefijo /{cidr} significa {cidr} bits en 1 para la máscara\n"
    output += "   • La dirección de red se obtiene con AND entre IP y máscara\n"
    output += "   • El broadcast se obtiene sumando la wildcard a la red\n"
    output += "   • El salto de bloque es 256 - (último octeto de máscara no 255)\n\n"
    
    output += "Completa los campos y presiona 'Verificar Respuestas' cuando termines.\n"
    
    practice_feedback.insert('1.0', output)

def start_subnet_practice():
    """Start subnet calculation practice"""
    ...
    if base_cidr == 16:
        ...
        ip = f"192.168.{random.randint(1, 254)}.0"
    ...
    # Clear fields
    practice_subnet_bits_entry.delete(0, tk.END)
    practice_subnet_newmask_entry.delete(0, tk.END)
    practice_subnet_hosts_entry.delete(0, tk.END)
    
    practice_feedback.delete('1.0', tk.END)
    
    output = "╔════════════════════════════════════════════════════════════════════════════╗\n"
    output += "║                   EJERCICIO DE PRÁCTICA DE SUBREDES                       ║\n"
    output += "╚════════════════════════════════════════════════════════════════════════════╝\n\n"
    
    output += f"📝 EJERCICIO:\n"
    output += f"   Red base: {ip}/{base_cidr}\n"
    output += f"   Número de subredes a crear: {num_subnets}\n\n"
    
    output += "🎯 TU TAREA:\n"
    output += "   Calcula manualmente:\n\n"
    
    output += "   1️⃣  ¿Cuántos BITS necesitas tomar prestados?\n"
    output += "   2️⃣  ¿Cuál será la NUEVA MÁSCARA? (formato: /XX)\n"
    output += "   3️⃣  ¿Cuántos HOSTS habrá por subred?\n\n"
    
    output += "💡 AYUDA:\n"
    output += f"   • Usa la fórmula: 2^n >= {num_subnets} para encontrar n (bits)\n"
    output += f"   • Nueva máscara = /{base_cidr} + bits prestados\n"
    output += "   • Hosts por subred = 2^(32 - nueva_máscara) - 2\n\n"
    
    output += "Completa los campos y presiona 'Verificar Subredes' cuando termines.\n"
    
    practice_feedback.insert('1.0', output)

def verify_basic_answers():
    """Verify the user's answers for basic practice"""
    if practice_state['mode'] != 'basic':
        messagebox.showwarning("Aviso", "Primero inicia un ejercicio básico")
        return
    
    network = practice_state['network']
    
    # Get user answers
    user_mask = practice_mask_entry.get().strip()
    user_network = practice_network_entry.get().strip()
    user_broadcast = practice_broadcast_entry.get().strip()
    user_block = practice_block_entry.get().strip()
    user_first_host = practice_first_host_entry.get().strip()
    user_last_host = practice_last_host_entry.get().strip()
    
    # Correct answers
    correct_mask = str(network.netmask)
    correct_network = str(network.network_address)
    correct_broadcast = str(network.broadcast_address)
    
    # Calculate block size
    mask_octets = [int(x) for x in correct_mask.split('.')]
    determining_octet = -1
    for i in range(3, -1, -1):
        if mask_octets[i] != 255:
            determining_octet = i
            break
    correct_block = str(256 - mask_octets[determining_octet] if determining_octet >= 0 else 256)
    
    correct_first_host = str(network.network_address + 1)
    correct_last_host = str(network.broadcast_address - 1)
    ...
    if user_mask == correct_mask:
        practice_mask_entry.config(bg='lightgreen')
        results.append("✅ Máscara de red: CORRECTO")
    else:
        practice_mask_entry.config(bg='lightcoral')
        results.append(f"❌ Máscara de red: INCORRECTO\n   Tu respuesta: {user_mask}\n   Respuesta correcta: {correct_mask}")
        all_correct = False
    
    # Network
    if user_network == correct_network:
        practice_network_entry.config(bg='lightgreen')
        results.append("✅ Dirección de red: CORRECTO")
    else:
        practice_network_entry.config(bg='lightcoral')
        results.append(f"❌ Dirección de red: INCORRECTO\n   Tu respuesta: {user_network}\n   Respuesta correcta: {correct_network}")
        all_correct = False
    
    # Broadcast
    if user_broadcast == correct_broadcast:
        practice_broadcast_entry.config(bg='lightgreen')
        results.append("✅ Broadcast: CORRECTO")
    else:
        practice_broadcast_entry.config(bg='lightcoral')
        results.append(f"❌ Broadcast: INCORRECTO\n   Tu respuesta: {user_broadcast}\n   Respuesta correcta: {correct_broadcast}")
        all_correct = False
    
    # Block size
    if user_block == correct_block:
        practice_block_entry.config(bg='lightgreen')
        results.append("✅ Salto de bloque: CORRECTO")
    else:
        practice_block_entry.config(bg='lightcoral')
        results.append(f"❌ Salto de bloque: INCORRECTO\n   Tu respuesta: {user_block}\n   Respuesta correcta: {correct_block}")
        all_correct = False
    
    # First host
    if user_first_host == correct_first_host:
        practice_first_host_entry.config(bg='lightgreen')
        results.append("✅ Primer host: CORRECTO")
    else:
        practice_first_host_entry.config(bg='lightcoral')
        results.append(f"❌ Primer host: INCORRECTO\n   Tu respuesta: {user_first_host}\n   Respuesta correcta: {correct_first_host}")
        all_correct = False
    
    # Last host
    if user_last_host == correct_last_host:
        practice_last_host_entry.config(bg='lightgreen')
        results.append("✅ Último host: CORRECTO")
    else:
        practice_last_host_entry.config(bg='lightcoral')
        results.append(f"❌ Último host: INCORRECTO\n   Tu respuesta: {user_last_host}\n   Respuesta correcta: {correct_last_host}")
        all_correct = False
    ...
    output = "=" * 80 + "\n"
    output += "RESULTADOS DE TU PRÁCTICA\n"
    output += "=" * 80 + "\n\n"
    ...
    if all_correct:
        ...
        output += "Has dominado el cálculo de parámetros de red.\n\n"
        output += "Presiona 'Nuevo Ejercicio Básico' para practicar más.\n"
    else:
        output += "\n📚 Revisa las respuestas incorrectas marcadas en rojo.\n"
        output += "Puedes intentar corregirlas y verificar de nuevo.\n"
    
    practice_feedback.insert('1.0', output)

def verify_subnet_answers():
    """Verify the user's answers for subnet practice"""
    ...
    if user_bits == correct_bits:
        ...
        results.append("✅ Bits necesarios: CORRECTO")
    else:
        practice_subnet_bits_entry.config(bg='lightcoral')
        results.append(f"❌ Bits necesarios: INCORRECTO\n   Tu respuesta: {user_bits}\n   Respuesta correcta: {correct_bits}\n   Explicación: 2^{correct_bits} = {2**int(correct_bits)} >= {num_subnets}")
        all_correct = False
    
    if user_newmask == correct_newmask:
        practice_subnet_newmask_entry.config(bg='lightgreen')
        results.append("✅ Nueva máscara: CORRECTO")
    else:
        practice_subnet_newmask_entry.config(bg='lightcoral')
        results.append(f"❌ Nueva máscara: INCORRECTO\n   Tu respuesta: {user_newmask}\n   Respuesta correcta: {correct_newmask}\n   Explicación: /{base_cidr} + {correct_bits} = {correct_newmask}")
        all_correct = False
    
    if user_hosts == correct_hosts:
        practice_subnet_hosts_entry.config(bg='lightgreen')
        results.append("✅ Hosts por subred: CORRECTO")
    else:
        practice_subnet_hosts_entry.config(bg='lightcoral')
        results.append(f"❌ Hosts por subred: INCORRECTO\n   Tu respuesta: {user_hosts}\n   Respuesta correcta: {correct_hosts}\n   Explicación: 2^{32 - (base_cidr + int(correct_bits))} - 2 = {correct_hosts}")
        all_correct = False
    ...
    output = "=" * 80 + "\n"
    output += "RESULTADOS DE TU PRÁCTICA DE SUBREDES\n"
    output += "=" * 80 + "\n\n"
    ...
    if all_correct:
        output += "\n🎉 ¡EXCELENTE! Todas tus respuestas son correctas.\n"
        output += "Has dominado el cálculo de subredes.\n\n"
        output += "Presiona 'Nuevo Ejercicio de Subredes' para practicar más.\n"
    else:
        output += "\n📚 Revisa las respuestas incorrectas marcadas en rojo.\n"
        output += "Lee las explicaciones y vuelve a intentarlo.\n"
    
    practice_feedback.insert('1.0', output)

# Practice tab layout
practice_main_frame = tk.Frame(tab3, padx=20, pady=20)
practice_main_frame.pack(fill=tk.BOTH, expand=True)

# Title
title_label = tk.Label(practice_main_frame, 
                       text="🎓 PRÁCTICA MANUAL GUIADA",
                       font=('Arial', 16, 'bold'),
                       fg='#2196F3')
title_label.pack(pady=10)

subtitle_label = tk.Label(practice_main_frame,
                         text="Aprende calculando manualmente con retroalimentación inmediata",
                         font=('Arial', 10, 'italic'))
subtitle_label.pack(pady=5)

# Separator
ttk.Separator(practice_main_frame, orient='horizontal').pack(fill=tk.X, pady=15)

# Exercise selection buttons
button_frame = tk.Frame(practice_main_frame)
button_frame.pack(pady=10)

btn_basic_practice = tk.Button(button_frame, 
                               text="📝 Nuevo Ejercicio Básico",
                               command=start_basic_practice,
                               bg='#4CAF50', fg='white',
                               font=('Arial', 11, 'bold'),
                               padx=20, pady=10)
btn_basic_practice.pack(side=tk.LEFT, padx=10)

btn_subnet_practice = tk.Button(button_frame,
                               text="🔢 Nuevo Ejercicio de Subredes",
                               command=start_subnet_practice,
                               bg='#2196F3', fg='white',
                               font=('Arial', 11, 'bold'),
                               padx=20, pady=10)
btn_subnet_practice.pack(side=tk.LEFT, padx=10)

# Practice area
practice_area = tk.Frame(practice_main_frame)
...
# Basic practice fields
basic_fields_frame = tk.LabelFrame(input_frame, text="Ejercicio Básico", padx=10, pady=10)
basic_fields_frame.pack(fill=tk.X, pady=5)

tk.Label(basic_fields_frame, text="Máscara de red:", anchor='w').grid(row=0, column=0, sticky='w', pady=3)
practice_mask_entry = tk.Entry(basic_fields_frame, width=20, font=('Arial', 10))
practice_mask_entry.grid(row=0, column=1, pady=3, padx=5)

tk.Label(basic_fields_frame, text="Dirección de red:", anchor='w').grid(row=1, column=0, sticky='w', pady=3)
practice_network_entry = tk.Entry(basic_fields_frame, width=20, font=('Arial', 10))
practice_network_entry.grid(row=1, column=1, pady=3, padx=5)

tk.Label(basic_fields_frame, text="Broadcast:", anchor='w').grid(row=2, column=0, sticky='w', pady=3)
practice_broadcast_entry = tk.Entry(basic_fields_frame, width=20, font=('Arial', 10))
practice_broadcast_entry.grid(row=2, column=1, pady=3, padx=5)

tk.Label(basic_fields_frame, text="Salto de bloque:", anchor='w').grid(row=3, column=0, sticky='w', pady=3)
practice_block_entry = tk.Entry(basic_fields_frame, width=20, font=('Arial', 10))
practice_block_entry.grid(row=3, column=1, pady=3, padx=5)

tk.Label(basic_fields_frame, text="Primer host:", anchor='w').grid(row=4, column=0, sticky='w', pady=3)
practice_first_host_entry = tk.Entry(basic_fields_frame, width=20, font=('Arial', 10))
practice_first_host_entry.grid(row=4, column=1, pady=3, padx=5)

tk.Label(basic_fields_frame, text="Último host:", anchor='w').grid(row=5, column=0, sticky='w', pady=3)
practice_last_host_entry = tk.Entry(basic_fields_frame, width=20, font=('Arial', 10))
practice_last_host_entry.grid(row=5, column=1, pady=3, padx=5)

btn_verify_basic = tk.Button(basic_fields_frame, text="✓ Verificar Respuestas",
                            command=verify_basic_answers,
                            bg='#FF9800', fg='white',
                            font=('Arial', 10, 'bold'),
                            padx=15, pady=5)
btn_verify_basic.grid(row=6, column=0, columnspan=2, pady=10)

# Subnet practice fields
subnet_fields_frame = tk.LabelFrame(input_frame, text="Ejercicio de Subredes", padx=10, pady=10)
subnet_fields_frame.pack(fill=tk.X, pady=5)

tk.Label(subnet_fields_frame, text="Bits necesarios:", anchor='w').grid(row=0, column=0, sticky='w', pady=3)
practice_subnet_bits_entry = tk.Entry(subnet_fields_frame, width=20, font=('Arial', 10))
practice_subnet_bits_entry.grid(row=0, column=1, pady=3, padx=5)

tk.Label(subnet_fields_frame, text="Nueva máscara:", anchor='w').grid(row=1, column=0, sticky='w', pady=3)
practice_subnet_newmask_entry = tk.Entry(subnet_fields_frame, width=20, font=('Arial', 10))
practice_subnet_newmask_entry.grid(row=1, column=1, pady=3, padx=5)

tk.Label(subnet_fields_frame, text="Hosts por subred:", anchor='w').grid(row=2, column=0, sticky='w', pady=3)
practice_subnet_hosts_entry = tk.Entry(subnet_fields_frame, width=20, font=('Arial', 10))
practice_subnet_hosts_entry.grid(row=2, column=1, pady=3, padx=5)

btn_verify_subnet = tk.Button(subnet_fields_frame, text="✓ Verificar Subredes",
                             command=verify_subnet_answers,
                             bg='#FF9800', fg='white',
                             font=('Arial', 10, 'bold'),
                             padx=15, pady=5)
btn_verify_subnet.grid(row=3, column=0, columnspan=2, pady=10)

# Right side - Feedback
feedback_frame = tk.LabelFrame(practice_area, text="💬 Instrucciones y Retroalimentación",
...
practice_feedback = tk.Text(feedback_frame, height=25, wrap=tk.WORD,
                           font=('Courier New', 9),
                           yscrollcommand=practice_scrollbar.set)
...
╔════════════════════════════════════════════════════════════════════════════╗
║                      BIENVENIDO A LA PRÁCTICA GUIADA                       ║
╚════════════════════════════════════════════════════════════════════════════╝

🎯 OBJETIVO:
   Aprender a calcular parámetros de red de forma manual con retroalimentación
   inmediata sobre tus respuestas.

📚 TIPOS DE EJERCICIOS:

   1️⃣  EJERCICIO BÁSICO:
       • Calcula máscara, red, broadcast, salto de bloque y rango de hosts
       • Practica operaciones AND, conversión binario-decimal
       • Ideal para principiantes

   2️⃣  EJERCICIO DE SUBREDES:
       • Calcula bits necesarios para crear N subredes
       • Determina la nueva máscara de subred
       • Calcula hosts disponibles por subred
       • Nivel intermedio

🚀 CÓMO EMPEZAR:

   1. Presiona uno de los botones superiores para generar un ejercicio
   2. Lee el enunciado cuidadosamente
   3. Calcula manualmente (usa papel y lápiz si lo necesitas)
   4. Ingresa tus respuestas en los campos de la izquierda
   5. Presiona "Verificar" para obtener retroalimentación
   6. Los campos correctos se marcarán en VERDE ✅
   7. Los campos incorrectos se marcarán en ROJO ❌ con explicación

💡 CONSEJOS:
   • Tómate tu tiempo para pensar en cada paso
   • Si te equivocas, lee la explicación y vuelve a intentar
   • Practica varios ejercicios para dominar los conceptos
   • Usa las otras pestañas si necesitas ver ejemplos detallados

¡Presiona un botón arriba para comenzar tu práctica!
"""

practice_feedback.insert('1.0', initial_instructions)
